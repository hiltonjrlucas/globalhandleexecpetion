let message = null;

var error = (response) => {
    CheckError(response);
};

$(document).ready(function () {
    DevExpress.localization.locale(navigator.language || navigator.browserLanguage);
    $(document).ajaxStop(function () {
        ShowLoadPanel('', false);
    });

    let usuario = sessionStorage.getItem('usuario');
    let url = window.location.pathname.split('/')[1];

    if (url && !['Curriculum', 'Conta'].includes(url)) {
        if (usuario) {
            GetUsuario(usuario);
        }
        else {
            window.location.href = urlApplication;
        }

        $('#logout').click(function () {
            Logout();
        });
    }

    moment.updateLocale('pt-br', {
        months: [
            "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho",
            "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"
        ],
        weekdays: [
            "Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"
        ],
        weekdaysShort: ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"]
    });
});

function GetUsuario(login) {
    let fotoUsuario = sessionStorage.getItem('fotoUsuario');
    let nomeUsuario = sessionStorage.getItem('nomeUsuario');
    let cargo = sessionStorage.getItem('cargo');

    if (fotoUsuario && nomeUsuario) {
        imgUsuario.setAttribute('src', 'data:image/jpg;base64,' + fotoUsuario);
        imgUsuarioExpandido.setAttribute('src', 'data:image/jpg;base64,' + fotoUsuario);

        $('#spanUsuario')[0].innerText = nomeUsuario;
        $('#spnFullUsuario')[0].innerText = nomeUsuario;
        $('#smallFuncao')[0].innerText = cargo;
    }
    else {
        const success = (response) => {
            if (response.success) {
                if (response.data.foto) {
                    sessionStorage.setItem('fotoUsuario', response.data.foto);

                    imgUsuario.setAttribute('src', 'data:image/jpg;base64,' + response.data.foto);
                    imgUsuarioExpandido.setAttribute('src', 'data:image/jpg;base64,' + response.data.foto);

                    $('img[alt=imgUsuario]').attr('src', 'data:image/jpg;base64,' + response.data.foto);
                    $('img[alt=imgUsuarioExpandido]').attr('src', 'data:image/jpg;base64,' + response.data.foto);
                }
                else {
                    sessionStorage.setItem('fotoUsuario', '');
                }

                $('#spanUsuario')[0].innerText = response.data.nom_pessoa_fisic;
                $('#spnFullUsuario')[0].innerText = response.data.nom_pessoa_fisic;
                $('#smallFuncao')[0].innerText = response.data.des_cargo_basic_funcao;

                sessionStorage.setItem('nomeUsuario', response.data.nom_pessoa_fisic);
                sessionStorage.setItem('cargo', response.data.des_cargo_basic_funcao);
                sessionStorage.setItem('setor', response.data.des_area);
                sessionStorage.setItem('admissao', response.data.dat_admis_func);

                $('#spanBemVindo').text(response.data.nom_pessoa_fisic);
            }
            else {
                sessionStorage.setItem('fotoUsuario', '');
            }
        };

        const errorGetUsuario = (response) => {
            imgUsuario.setAttribute('src', "data:image/png;base64,");
        };

        const data = { url: '/bcu/funcionario/' + login };
        SaudeSegurancaGet(data, success, errorGetUsuario);
    }
}

function Logout() {
    const success = (response) => {
        if (response.success) {
            sessionStorage.clear();
            window.location.href = urlApplication;
        }
        else {
            CheckError(response);
        }
    };

    let url = '/Home/Logout';
    Post(url, '', success, error);
}

function MoneyFormat(amount, decimalCount, decimal, thousands) {
    return Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(amount);
}

function ExportFile(data) {
    saveAs(base64ToBlob(data.content, `${data.contentType};charset=utf-8`), data.fileName);
}

function CheckClaim(type, value) {
    return JSON.parse(sessionStorage.getItem('claims')).find(f => f.type === type).value === value;
}

function base64ToBlob(data, contentType, sliceSize) {
    contentType = contentType || "";
    sliceSize = sliceSize || 512;
    data = data.replace(/\s/g, ''); //IE compatibility...
    let byteCharacters = atob(data);
    let byteArrays = [];
    for (let offset = 0; offset < byteCharacters.length; offset += sliceSize) {
        let slice = byteCharacters.slice(offset, offset + sliceSize);

        let byteNumbers = new Array(slice.length);
        for (let i = 0; i < slice.length; i++) {
            byteNumbers[i] = slice.charCodeAt(i);
        }
        let byteArray = new Uint8Array(byteNumbers);
        byteArrays.push(byteArray);
    }
    return new Blob(byteArrays, { type: contentType });
}

function CheckError(response) {
    if (response.message) {
        if (Array.isArray(response.message)) {
            //Tratando mensagens geradas por Notificacoes
            ShowMessage('error', response.message.join("\n\n"));
            return;
        } else {
            if (typeof (response.message) === "string") {
                if (response.message.includes('TokenInvalido')) {

                    ShowMessage('error', 'Houve um problema com a sessão. Faça login novamente!');
                    setTimeout(function () {
                        Logout();
                    }, 5000);
                    return;
                }

                ShowMessage('error', response.message);
                return;
            } else {
                //Tratando mensagens geradas por Annotations
                var msgs = [];
                for (var key in response.message) {
                    msgs = msgs.concat(response.message[key]);
                }
                ShowMessage('error', msgs.join("\n\n"));
                return;
            }
        }
    } else {
        if (response.http500Message) {

            ShowDialog("Ocorreu uma falha no processamento!<br><br>Caso o erro persista,<br>favor acionar o Suporte Vicunha.<br>" + response.http500Message, () => { }, DialogTypes.error);
            return;
        } else {
            if (response.statusText && response.statusText === "error") {
                ShowDialog("Ocorreu uma falha no processamento!<br><br>Caso o erro persista,<br>favor acionar o Suporte Vicunha.<br>", () => { }, DialogTypes.error);
                return;
            }
        }
    }
}

function GetUserYears() {
    let current = new Date().getFullYear();
    const end = (new Date(sessionStorage.getItem('admissao'))).getFullYear();
    const data = [];

    while (current >= end) {
        data.push({ value: current });

        current--;
    }

    return data;
}

function GetSelectBox(data) {
    return data.map((item, index) => {
        return { exibicao: item, valor: index + 1 };
    });
}

function GetEstados() {
    return [
        { estado: 'Acre', sigla: 'AC' },
        { estado: 'Alagoas', sigla: 'AL' },
        { estado: 'Amapá', sigla: 'AP' },
        { estado: 'Amazonas', sigla: 'AM' },
        { estado: 'Bahia', sigla: 'BA' },
        { estado: 'Ceará', sigla: 'CE' },
        { estado: 'Distrito Federal', sigla: 'DF' },
        { estado: 'Espírito Santo', sigla: 'ES' },
        { estado: 'Goiás', sigla: 'GO' },
        { estado: 'Maranhão', sigla: 'MA' },
        { estado: 'Mato Grosso', sigla: 'MT' },
        { estado: 'Mato Grosso do Sul', sigla: 'MS' },
        { estado: 'Minas Gerais', sigla: 'MG' },
        { estado: 'Pará', sigla: 'PA' },
        { estado: 'Paraíba', sigla: 'PB' },
        { estado: 'Paraná', sigla: 'PR' },
        { estado: 'Pernambuco', sigla: 'PE' },
        { estado: 'Piauí', sigla: 'PI' },
        { estado: 'Rio de Janeiro', sigla: 'RJ' },
        { estado: 'Rio Grande do Norte', sigla: 'RN' },
        { estado: 'Rio Grande do Sul', sigla: 'RS' },
        { estado: 'Rondônia', sigla: 'RO' },
        { estado: 'Roraima', sigla: 'RR' },
        { estado: 'Santa Catarina', sigla: 'SC' },
        { estado: 'São Paulo', sigla: 'SP' },
        { estado: 'Sergipe', sigla: 'SE' },
        { estado: 'Tocantins', sigla: 'TO' }
    ];
}

function GetEndereco(cep) {
    const success = (response) => {
        if (response) {
            return response;
        } else {
            ShowMessage('error', 'CEP não localizado.');
        }
    };
    const error = () => {
        ShowMessage('error', 'CEP inválido.');
    };

    return $.ajax({
        type: 'GET',
        url: `https://viacep.com.br/ws/${cep}/json/`,
        dataType: 'json',
        success: success,
        error: error
    });
}

function ValidateCpf(cpf) {
    cpf = cpf.replace(/[^\d]+/g, '');
    if (cpf === '') {
        return false;
    }

    // Elimina CPFs invalidos conhecidos	
    if (cpf.length !== 11 ||
        [
            '00000000000', '11111111111', '22222222222', '33333333333', '44444444444',
            '55555555555', '66666666666', '77777777777', '88888888888', '99999999999'
        ].includes(cpf)) {
        return false;
    }

    // Valida 1o digito	
    let sum = 0;
    for (i = 0; i < 9; i++) {
        sum += parseInt(cpf.charAt(i)) * (10 - i);
    }

    let rest = 11 - (sum % 11);
    if (rest === 10 || rest === 11) {
        rest = 0;
    }
    if (rest !== parseInt(cpf.charAt(9))) {
        return false;
    }

    // Valida 2o digito	
    sum = 0;
    for (i = 0; i < 10; i++) {
        sum += parseInt(cpf.charAt(i)) * (11 - i);
    }

    rest = 11 - (sum % 11);
    if (rest === 10 || rest === 11) {
        rest = 0;
    }
    if (rest !== parseInt(cpf.charAt(10))) {
        return false;
    }

    return true;
}

function CheckMobile() {
    if (/mobi/i.test(navigator.userAgent)) {
        return true;
    } else {
        return false;
    }
}

function arrayBufferToBase64(buffer) {
    var binary = '';
    var bytes = new Uint8Array(buffer);
    var len = bytes.byteLength;
    for (var i = 0; i < len; i++) {
        binary += String.fromCharCode(bytes[i]);
    }
    return btoa(binary);
}